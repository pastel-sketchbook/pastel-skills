# Axum Web Service Patterns — Reference
...
## 8. Middleware and Context Logging with UUID v7
...
- Custom error type implements `IntoResponse` for non-trivial APIs. Do not leak internal error details.
- Use `anyhow::Result` in `main.rs` and domain code; use `thiserror` for typed errors that cross module boundaries.

### Extractors and handlers
- Extractor ordering: `State`, `Path`, `Query`, `HeaderMap`, `Json` (body last).
- `State` wraps `Arc` with `AppState` — never a raw struct.
- `#[tracing::instrument(skip(state), err)]` on every public handler.
- Structured tracing fields — not format strings.
- No `println!` / `eprintln!` — use `tracing::*` exclusively.

### Concurrency and async
- No `Arc` with `Mutex` held across `.await`. Use `tokio::sync::Mutex` if a guard must span await points.
- No blocking I/O in async context. Use tokio equivalents or `spawn_blocking`.
- `tokio::select!` branches must be cancellation-safe.
